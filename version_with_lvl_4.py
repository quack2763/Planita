...
def level_selection_menu():
    level1 = MenuImageButton(menu_lvl1_image, (135, 245), (210, 155))
    level2 = MenuImageButton(menu_lvl2_image, (380, 245), (210, 155))
    level3 = MenuImageButton(menu_lvl3_image, (620, 245), (210, 155))
    level4 = MenuImageButton(menu_lvl4_image, (865, 245), (210, 155))
    buttons = [level1, level2, level3, level4]
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                ...
                if level1.pressed and level1.hit(event.pos):
                    ...
                    selected = 3
                elif level4.pressed and level4.hit(event.pos):
                    selected = 4
        ...
        draw_menu_title("SELECT LEVEL")

        for button in buttons:
            button.draw(SCREEN)

        for number, x in enumerate((135, 380, 620, 865), 1):
...
while running:
    ...
    if (
        ...
        for moving_platform in moving_platforms:
            ...
            if player.gravity_direction == 1:
                standing_on_platform = (
                    player_rect_before.bottom <= platform_rect.top + 8
                    and player_rect_before.bottom >= platform_rect.top - 8
                    and player_rect_before.right > platform_rect.left
                    and player_rect_before.left < platform_rect.right
                    and player.vy >= 0
                )
            else:
                standing_on_platform = (
                    player_rect_before.top >= platform_rect.bottom - 8
                    and player_rect_before.top <= platform_rect.bottom + 8
                    and player_rect_before.right > platform_rect.left
                    and player_rect_before.left < platform_rect.right
                    and player.vy <= 0
                )
        ...
        if not dead:
            ...
            for enemy in flying_enemies:
                ...
                if enemy.alive and player_hits_enemy(
                    player.rect,
                    enemy.hitbox
                ):
                    dead = True
    ...
    if current_level == 4:
        draw_level4_background(SCREEN, camera_x)
    else:
